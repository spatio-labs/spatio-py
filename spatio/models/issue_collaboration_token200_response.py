...
class IssueCollaborationToken200Response(BaseModel):
    """
    IssueCollaborationToken200Response
    """ # noqa: E501
    token: StrictStr = Field(description="HS256 JWT, signed with the shared platform/worker secret.")
    ws_url: StrictStr = Field(description="Base WebSocket URL of the Yjs worker.")
    room: Optional[StrictStr] = None
    expires_at: datetime
    expires_in: StrictInt = Field(description="Seconds until the token expires.")
    __properties: ClassVar[List[str]] = ["token", "ws_url", "room", "expires_at", "expires_in"]
